...
const SLIDERS = [
  { key: "warmth",    label: "Calidez",  min: 1, max: 10, default: 5, left: "Neutra",       right: "Muy calida" },
  { key: "closeness", label: "Cercania", min: 1, max: 10, default: 5, left: "Profesional",  right: "Intima"     },
  { key: "pace",      label: "Ritmo",    min: 1, max: 10, default: 4, left: "Muy pausado",  right: "Fluido"     },
];

export default function ParameterSliders({ params, onChange, apiKey }) {
    ...
    <div className="w-full space-y-6 bg-slate-900/50 border border-slate-800 rounded-2xl p-5">
      ...
      <div>
        ...
        <p className="mt-2 text-xs text-slate-600">
          ~{DURATION_PRESETS.find((p) => p.value === selectedDuration)?.words ?? 220} palabras
        </p>
      </div>
      ...
      <div>
        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
          Voz{apiKey && <span className="normal-case font-normal text-slate-600 ml-2">— presiona ▶ para escuchar</span>}
        </p>
              ...
              <div key={v} className={`flex rounded-lg overflow-hidden border ${activeBorder}`}>
                ...
                {apiKey && (
                  <button
                    ...
                    disabled={isLoading}
                    title={isPlaying ? "Detener" : isLoading ? "Cargando..." : "Escuchar"}
      ...
      <div>
        <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-3">
          Contexto personal <span className="normal-case font-normal text-slate-600">(opcional)</span>
        </label>
        <textarea
          value={params.context ?? ""}
          onChange={(e) => set("context", e.target.value)}
          placeholder="Ej: estoy pasando por un proceso laboral muy agresivo, tengo insomnio desde hace semanas..."
          rows={3}
