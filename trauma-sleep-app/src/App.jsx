...
export default function App() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setError(e.message || 'Ocurrio un error. Revisa tu API key y la conexion.');
      setStatus('error');
    }
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      <div className="max-w-2xl mx-auto px-4 py-10 space-y-10">
        ...
        <header className="text-center space-y-2">
          ...
          <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">
            Audios terapeuticos generados con IA para sostener el equilibrio cuando el sistema nervioso esta al limite.
          </p>
        </header>

        <div className="space-y-6 rounded-2xl border border-slate-800/80 bg-slate-900/30 p-5">
          <ApiKeyInput apiKey={apiKey} onKeySet={setApiKey} />
          <ModelSelector
        ...
        <div className="flex gap-1 bg-slate-900 rounded-xl p-1">
          ...
          >
            Biblioteca {library.length > 0 && <span className="text-indigo-400 ml-1">{library.length}</span>}
          </button>
        </div>
          ...
          <div className="space-y-8">
            {!apiKey && (
              <div className="bg-amber-950/40 border border-amber-800/40 rounded-2xl px-5 py-4 text-sm text-amber-400">
                Ingresa tu API key de OpenAI para empezar.
              </div>
            )}
            ...
            {selectedHeader && (
              <ParameterSliders params={params} onChange={setParams} apiKey={apiKey} />
            )}
            ...
            {selectedHeader && apiKey && (
              ...
              >
                {status === 'loading' && (
                  <svg
                    className="h-5 w-5 shrink-0 animate-spin text-white/90"
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    {tokenUsage && (
                      <div className="flex items-center gap-3 text-xs text-slate-600">
                        <span title="Tokens de entrada"><span className="text-slate-500">entrada</span> <span className="text-indigo-400 font-mono">{tokenUsage.prompt_tokens}</span></span>
                        <span title="Tokens generados"><span className="text-slate-500">salida</span> <span className="text-indigo-400 font-mono">{tokenUsage.completion_tokens}</span></span>
                        <span title="Total"><span className="text-slate-500">total</span> <span className="text-indigo-400 font-mono">{tokenUsage.total_tokens}</span></span>
                      </div>
                    )}
                    <button
                    ...
                    >
                      ...
                      {playlist.some((e) => e.id === result.id) ? 'En playlist' : 'Agregar a playlist'}
                    </button>
                  </div>
                </div>
                <AudioPlayer
        ...
        <footer className="text-center text-xs text-slate-700 pb-4">
          Los audios se guardan localmente en tu navegador. Tu API key nunca sale de este dispositivo.
        </footer>
      </div>
      ...
      {playlist.length > 0 && !showPlaylist && (
        ...
        >
          ...
          Playlist <span className="bg-white/20 rounded-lg px-1.5 py-0.5 text-xs">{playlist.length}</span>
        </button>
      )}
