...
export const DURATION_PRESETS = [
  { value: 1, label: "~1 min", words: 240 },
  { value: 2, label: "~2 min", words: 440 },
  { value: 3, label: "~5 min", words: 900 },
  { value: 5, label: "~10 min", words: 2250 },
  { value: 8, label: "~20 min", words: 4500 },
];

export function durationToMaxTokens(durationMinutes) {
  const preset =
    DURATION_PRESETS.find((p) => p.value === durationMinutes) ??
    DURATION_PRESETS[1];
...
function applyParams(systemBase, params) {
  ...
  const preset =
    DURATION_PRESETS.find((p) => p.value === duration) ?? DURATION_PRESETS[1];

  const warmthDesc =
    warmth <= 3
      ? "Tono neutro y contenido, sin exceso de emoción."
      : warmth <= 6
        ? "Tono cálido y acompañante."
        : "Tono muy cálido, íntimo, como un abrazo verbal. Máxima ternura posible sin perder autenticidad.";

  const closenessDesc =
    closeness <= 3
      ? "Tratamiento respetuoso, algo de distancia, como un profesional de confianza."
      : closeness <= 6
        ? "Tratamiento cercano, directo, como un amigo que te conoce bien."
        : "Tratamiento completamente íntimo. Hablás como si fueras la voz interior más honesta de la persona.";

  const paceDesc =
    pace <= 3
      ? "Ritmo muy pausado. Frases cortas. Muchos puntos y comas. Respiración entre cada idea."
      : pace <= 6
        ? "Ritmo moderado. Fluido pero sin apuro."
